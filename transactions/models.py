...
class FailedSMSLog(models.Model):
    sms_body = models.TextField()
    reason = models.CharField(max_length=255, null=True, blank=True)
    processed_at = models.DateTimeField(auto_now_add=True)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null=True, blank=True, related_name='failed_sms_logs')

    def __str__(self):
        return f"Failed SMS for {self.user} at {self.processed_at}: {self.sms_body[:50]}..."
